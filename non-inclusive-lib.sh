...
function search_non_inclusive_words() {
    ...
    for nword in "${non_inclusive_words[@]}"; do
        ...
        grep_output=$(grep $grep_options $exclude_patterns "$nword" "$folder_path"/* 2>/dev/null)
    ...
    time_taken=$((end_time - start_time))  # Calculate time taken using arithmetic expansion
